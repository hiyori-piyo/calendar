...
### アップデート (2026/02/20)
データの管理方法を拡張したバージョンを作成しました。

- **feat/supabase-sql-fetch ブランチ**
  - これまでの `LocalStorage` による一時的な保存ではなく、`Supabase (SQL)` を利用してデータを永続化する仕組みを導入しています。
  - 編集・削除機能を追加しました。
  - サーバーサイドでのデータ取得・管理の学習を目的としています。
